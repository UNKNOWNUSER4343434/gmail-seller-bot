...
""")
cursor.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('rate', '15.0')")
conn.commit()
...
def set_rate(new_rate):
  cursor.execute("UPDATE settings SET value=? WHERE key='rate'", (str(new_rate),))
  conn.commit()

def get_available_stock_count():
  cursor.execute("SELECT COUNT(*) FROM task_stock WHERE status='available'")
...
def get_main_menu():
  return ReplyKeyboardMarkup(
      keyboard=[
          ...
          [KeyboardButton(text="💼 Wallet & Balance"), KeyboardButton(text="💸 Request Payout")],
          [KeyboardButton(text="📢 Updates Channel"), KeyboardButton(text="🛠 24/7 Support")],
      ],
...
async def updates_handler(message: types.Message):
  await message.answer(f"📢 **Official Announcement Channel:**\nJoin here: {CHANNEL_LINK}", disable_web_page_preview=True)

@dp.message(F.text == "🛠 24/7 Support")
async def support_handler(message: types.Message):
  await message.answer(f"🛠 **Customer Support:**\nFor any inquiries or issues, contact: {SUPPORT_USER}")

@dp.message(F.text == "💼 Wallet & Balance")
async def wallet_handler(message: types.Message):
  cursor.execute("SELECT balance, total_submitted FROM users WHERE user_id=?", (message.from_user.id,))
  row = cursor.fetchone()
...
async def choose_method(message: types.Message):
  markup = InlineKeyboardMarkup(
      inline_keyboard=[
          [InlineKeyboardButton(text="📁 Sell Readymade Account", callback_data="sell_ready")],
          [InlineKeyboardButton(text="📋 Create From Bot Data", callback_data="sell_botdata")],
      ]
  )
  await message.answer("Please select how you would like to submit the account:", reply_markup=markup)
...
async def start_readymade(call: types.CallbackQuery, state: FSMContext):
  ...
  await call.message.answer("📧 **Enter your Gmail Address:**\n*(Example: john.doe982@gmail.com)*", parse_mode="Markdown")
...
async def start_botdata(call: types.CallbackQuery, state: FSMContext):
  ...
  cursor.execute("SELECT id, email, password FROM task_stock WHERE status='available' LIMIT 1")
  item = cursor.fetchone()

  if not item:
    await call.message.answer(
        "⚠️ **Task Data Out of Stock!**\n\n"
        "Currently, there is no task data available. Please wait a short while. Our team will add fresh tasks shortly."
    )
    await bot.send_message(
        chat_id=ADMIN_ID,
        text="🚨 **STOCK ALERT:**\nA user tried to request task data, but the stock is **EMPTY (0)**!\nUse `/admin` -> **Upload Task Data** to restock.",
        parse_mode="Markdown",
    )
    await call.answer()
    return
  ...
  cursor.execute("UPDATE task_stock SET status='assigned', assigned_to=? WHERE id=?", (user_id, stock_id))
  conn.commit()

  remaining = get_available_stock_count()
  if remaining == 0:
    await bot.send_message(chat_id=ADMIN_ID, text="⚠️ **Stock Warning:** The last task data was just assigned! Available stock is now 0.")
  ...
  msg = (
      ...
      "📌 *Instructions:*\n"
      "1. Create a new Google Account using the **exact email and password** provided above.\n"
...
async def process_email(message: types.Message, state: FSMContext):
  ...
  if "@gmail.com" not in email.lower():
    await message.answer("⚠️ **Invalid Format:** Please enter a valid Gmail address ending with `@gmail.com`:")
    return
...
async def process_password(message: types.Message, state: FSMContext):
  ...
  skip_kb = InlineKeyboardMarkup(
      inline_keyboard=[[InlineKeyboardButton(text="⏭ Skip Recovery Email", callback_data="skip_recovery")]]
  )
  await message.answer(
      "🛡 **Recovery Email Check:**\nDid you attach a recovery email? If yes, send it now. Otherwise, tap **Skip**.",
...
async def ask_2fa_step(message_obj, state: FSMContext):
  kb = InlineKeyboardMarkup(
      inline_keyboard=[
          [InlineKeyboardButton(text="🔐 Yes, Provide 2FA Key (High Approval)", callback_data="2fa_yes")],
          [InlineKeyboardButton(text="⏩ Submit Without 2FA", callback_data="2fa_no")],
      ]
  )
  info_text = (
      "🔐 **Two-Factor Authentication (2FA):**\n\n"
      "💡 *Tip:* Adding 2FA backup/secret keys drastically increases verification speed and prevents lockouts.\n\n"
...
async def ask_2fa_key(call: types.CallbackQuery, state: FSMContext):
  await call.message.answer("🔑 Please paste your **2FA Secret Key / Backup Code** below:")
...
async def final_submission(message_obj, user, state: FSMContext):
  ...
  cursor.execute("UPDATE users SET total_submitted = total_submitted + 1 WHERE user_id=?", (user.id,))
  conn.commit()

  admin_kb = InlineKeyboardMarkup(
      inline_keyboard=[[
          InlineKeyboardButton(text="✅ Approve", callback_data=f"adm_app_{sub_id}"),
          InlineKeyboardButton(text="❌ Reject", callback_data=f"adm_rej_{sub_id}"),
      ]]
  )
  ...
  await message_obj.answer(
      ...
      "🕒 Your details will be verified by our team within **24 to 72 hours** (up to 3 days).\n"
      "Once approved, your balance will automatically be credited to your wallet.",
      parse_mode="Markdown",
  )
  await state.clear()
...
async def admin_approve(call: types.CallbackQuery):
  ...
  cursor.execute("SELECT user_id, email, status FROM submissions WHERE id=?", (sub_id,))
  row = cursor.fetchone()
  ...
  cursor.execute("UPDATE users SET balance = balance + ? WHERE user_id=?", (rate, uid))
  conn.commit()

  try:
    await bot.send_message(
        chat_id=uid,
        text=f"🎉 **Congratulations!**\nYour account `{mail}` has been **Approved**.\n**₹{rate:.2f}** has been credited to your wallet!",
        parse_mode="Markdown",
    )
  except:
    pass

  await call.message.edit_text(f"{call.message.text}\n\n✅ **STATUS: APPROVED (+₹{rate:.2f})**", parse_mode="Markdown")
...
async def admin_reject(call: types.CallbackQuery):
  ...
  try:
    await bot.send_message(
        chat_id=uid,
        text=f"❌ **Submission Rejected:**\nYour account `{mail}` could not be approved due to invalid credentials, incorrect password, or activation lock.",
        parse_mode="Markdown",
    )
  except:
    pass
...
async def withdraw_menu(message: types.Message, state: FSMContext):
  ...
  await message.answer("📱 Please enter your **UPI ID** where you wish to receive payment:\n*(e.g. `9876543210@paytm`)*", parse_mode="Markdown")
...
async def process_withdraw(message: types.Message, state: FSMContext):
  ...
  pay_kb = InlineKeyboardMarkup(
      inline_keyboard=[[InlineKeyboardButton(text="💸 Mark as Paid", callback_data=f"paid_{uid}_{bal}")]]
  )
  ...
  await message.answer("✅ **Payout Request Received!**\nYour payment will be processed shortly.")
  await state.clear()
...
async def mark_paid(call: types.CallbackQuery):
  ...
  try:
    await bot.send_message(
        chat_id=int(uid),
        text=f"✅ **Payment Completed!**\nYour payout of **₹{amt}** has been sent to your UPI ID.",
        parse_mode="Markdown",
    )
  except:
    pass
  await call.message.edit_text(f"{call.message.text}\n\n✅ **STATUS: PAID & SETTLED**", parse_mode="Markdown")
...
async def admin_panel(message: types.Message):
  ...
  kb = InlineKeyboardMarkup(
      inline_keyboard=[
          [InlineKeyboardButton(text="📥 Upload Task Data (Stock)", callback_data="adm_upload_stock")],
          [InlineKeyboardButton(text="⚙️ Update Rate Per Mail", callback_data="adm_change_rate")],
          [InlineKeyboardButton(text="💳 Add/Deduct Balance", callback_data="adm_add_bal")],
      ]
  )
...
async def process_bulk_stock(message: types.Message, state: FSMContext):
  ...
  for line in lines:
    if ":" in line:
      ...
      pwd = parts[1].strip()
      try:
        cursor.execute("INSERT INTO task_stock (email, password) VALUES (?, ?)", (mail, pwd))
        added += 1
...
async def process_new_rate(message: types.Message, state: FSMContext):
  ...
  try:
    ...
    set_rate(new_val)
    await message.answer(f"✅ Rate successfully updated to: **₹{new_val:.2f}** per account.", parse_mode="Markdown")
  except ValueError:
